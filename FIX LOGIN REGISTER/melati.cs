...
namespace Ticket
{
    public partial class melati : Form
    {
        ...
        private void melati_Load(object sender, EventArgs e)
        {
            ...
            using (NpgsqlConnection conn = new NpgsqlConnection("Server=localhost; Port=5432; User Id=postgres; Password=; Database=Jecation"))
            {
                conn.Open();
                using NpgsqlCommand cmd = new NpgsqlCommand();
                cmd.Connection = conn;
                cmd.CommandText = "SELECT nama_fasilitas_kamar FROM fasilitas_kamar where id_kamar = 2 ORDER BY id_fasilitas_kamar ASC";
                ...
                if (reader.Read())
                {
                    ...
                    do
                    {
                        ...
                        Label label = this.Controls.Find("Melati" + labelIndex, true).FirstOrDefault() as Label;
