            ...
            if (result == DialogResult.Yes)
            {
                ...
                using (NpgsqlConnection conn = new NpgsqlConnection(connString))
                {
                    ...
                    try
                    {
                        ...
                        // Update status menjadi "true" untuk row yang dipilih
                        string updateStatusQuery = "UPDATE kode_pembayaran SET status = true WHERE id_kode_pembayaran = @id; insert into riwayat_penginapan(id_kamar, jumlah_malam, jumlah_kamar, total_harga, id_akun) select id_kamar, jumlah_malam, jumlah_kamar, total_harga, id_akun from notifikasi_admin";
                        cmd.CommandText = updateStatusQuery;
