...
namespace FIX_LOGIN_REGISTER
{
    public partial class detail_melatiI : Form
    {
        ...
        public detail_melatiI(Login.User user)
        {
            InitializeComponent();
            this.user = user;
        }
        ...
        private int GetSelisihHari()
        {
            ...
            DateTime checkInDate = dateTimePicker4.Value;
            DateTime checkOutDate = dateTimePicker3.Value.Date.AddDays(1).AddSeconds(-1);
        ...
        public void UpdateResult()
        {
            ...
            int nilaiLabel = Convert.ToInt32(label33.Text);
            int hasilPerkalian = selisihHari * jumlahPilihan * nilaiLabel;

            label36.Text = ("Total Harga = " + hasilPerkalian.ToString());
            label37.Text = ("x " + selisihHari.ToString() + " Malam");
            label35.Text = (("Tagihan Minimal = ") + hasilPerkalian * 1 / 4);
        }
        ...
        {
            UpdateResult(); label37.Visible = true; label36.Visible = true; label35.Visible = true;
        }
        ...
        private void button4_Click(object sender, EventArgs e)
        {
            ...
            using (NpgsqlConnection connection = new NpgsqlConnection("Host=localhost;Port=5432;Username=postgres;Password=;Database=Jecation"))
            {
                ...
                NpgsqlCommand command = new NpgsqlCommand();
                command.Connection = connection;
                command.CommandText = "insert into reservasi_penginapan (id_akun,jumlah_kamar,id_kamar, jadwal_masuk_penginapan, jadwal_keluar_penginapan, harga, harga_minimal , nama_kamar) values ( @id_akun,@jumlah_kamar, @id, @checkin, @checkout, @harga, @harga_minimal,'MelatiI')";
                command.Parameters.AddWithValue("@jumlah_kamar", jumlah);
                command.Parameters.AddWithValue("@checkin", dateTimePicker4.Value);
                command.Parameters.AddWithValue("@checkout", dateTimePicker3.Value);
