...
namespace FIX_LOGIN_REGISTER
{
    public partial class detail_mawarII : Form
    {
        ...
        public detail_mawarII(Login.User user)
        {
            InitializeComponent();
            this.user = user;
        }
        ...
        private void detail_mawarII_Load(object sender, EventArgs e)
        {
            try
            {
                int id = 9;
        ...
        private void button15_Click(object sender, EventArgs e)
        {
            ...
            using (NpgsqlConnection connection = new NpgsqlConnection("Host=localhost;Port=5432;Username=postgres;Password=;Database=Jecation"))
            {
                ...
                NpgsqlCommand command = new NpgsqlCommand();
                command.Connection = connection;
                command.CommandText = "insert into reservasi_penginapan (id_akun,jumlah_kamar,id_kamar, jadwal_masuk_penginapan, jadwal_keluar_penginapan, harga, harga_minimal , nama_kamar) values ( @id_akun,@jumlah_kamar, @id, @checkin, @checkout, @harga, @harga_minimal,'Mawr II')";
