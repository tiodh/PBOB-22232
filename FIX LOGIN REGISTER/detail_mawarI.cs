...
namespace FIX_LOGIN_REGISTER
{
    public partial class detail_mawarI : Form
    {
        ...
        public detail_mawarI(Login.User user)
        {
            InitializeComponent();
            this.user = user;
        }
        ...
        private int GetSelisihHari()
        {
            ...
            DateTime checkInDate = dateTimePicker10.Value;
            DateTime checkOutDate = dateTimePicker9.Value.Date.AddDays(1).AddSeconds(-1);
        ...
        public void UpdateResult()
        {
            ...
            int nilaiLabel = Convert.ToInt32(label98.Text);
            int hasilPerkalian = selisihHari * jumlahPilihan * nilaiLabel;


            label102.Text = ("Total Harga = " + hasilPerkalian.ToString());
            label101.Text = ("x " + selisihHari.ToString() + " Malam");
            label100.Text = (("Tagihan Minimal = ") + hasilPerkalian * 1 / 4);
        }
        ...
        private void detail_mawarI_Load_2(object sender, EventArgs e)
        {
            try
            {
                int id = 7;
                using (NpgsqlConnection connection = new NpgsqlConnection("Host=localhost;Port=5432;Username=postgres;Password=;Database=Jecation"))
                {
                    ...
                    {
                        checkedListBox2.Items.Add(nomorKamar);
                    }
        ...
        {
            UpdateResult(); label102.Visible = true; label101.Visible = true; label100.Visible = true;
        }
        ...
        private void button10_Click(object sender, EventArgs e)
        {
            ...
            using (NpgsqlConnection connection = new NpgsqlConnection("Host=localhost;Port=5432;Username=postgres;Password=;Database=Jecation"))
            {
                ...
                NpgsqlCommand command = new NpgsqlCommand();
                command.Connection = connection;
                command.CommandText = "insert into reservasi_penginapan (id_akun,jumlah_kamar,id_kamar, jadwal_masuk_penginapan, jadwal_keluar_penginapan, harga, harga_minimal , nama_kamar) values ( @id_akun,@jumlah_kamar, @id, @checkin, @checkout, @harga, @harga_minimal,'Mawar I')";
                command.Parameters.AddWithValue("@jumlah_kamar", jumlah);
                command.Parameters.AddWithValue("@checkin", dateTimePicker14.Value);
                command.Parameters.AddWithValue("@checkout", dateTimePicker13.Value);
